...
from grimoire.cli.helpers import async_command
...
async def mcp(stdio: bool, sse: bool, host: str, port: int) -> None:
    ...
    # Database lifecycle is handled by the MCP server's lifespan.
    # We only authenticate eagerly for stdio so tools can assume a valid key.
    if stdio:
