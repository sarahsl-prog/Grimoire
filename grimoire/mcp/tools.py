...
class AskInput(BaseModel):
    ...
    query: str = Field(
        ..., description="Question to answer.", min_length=1, max_length=4000
    )
...
class IngestDirectoryInput(BaseModel):
    ...
    directory: str = Field(
        ..., description="Absolute path to the directory.", min_length=1
    )
...
class GenerateInput(BaseModel):
    ...
    content_type: str = Field(
        ..., description="summary | flash_card | cliff_notes | outline | extract"
    )
...
class CveSearchInput(BaseModel):
    ...
    query: str | None = Field(
        default=None,
        max_length=2000,
        description="Semantic search over CVE descriptions.",
    )
...
class PlaybookSearchInput(BaseModel):
    ...
    platform: str | None = Field(
        default=None,
        max_length=64,
        description="Platform facet, e.g. 'windows', 'linux', 'aws'.",
    )
    ...
    @classmethod
    def _validate_phase(cls, v: str | None) -> str | None:
        if v is None:
            return None
    ...
    @classmethod
    def _validate_mitre_technique_id(cls, v: str | None) -> str | None:
        if v is None:
            return None
...
class PgQueryInput(BaseModel):
    ...
    sql: str = Field(
        ..., min_length=1, max_length=4000, description="SELECT query only."
    )
    limit: int = Field(default=50, ge=1, le=200)
    ...
    def _must_be_select(cls, v: str) -> str:
        ...
        if not (stripped.startswith("SELECT") or stripped.startswith("WITH")):
            raise ValueError(
                "Only SELECT queries (including WITH ... SELECT CTEs) are permitted."
            )
...
async def grimoire_search_cve(params: CveSearchInput, ctx: Context) -> str:
        ...
        return _ok(
            {
                ...
                "document": {
                    ...
                    "content_date": (
                        doc.content_date.isoformat() if doc.content_date else None
                    ),
                },
                "chunks": [
    ...
    if params.min_cvss is not None:
        ...
        conditions.append(
            cast(func.json_extract(Document.security_metadata, "$.cvss_score"), Float)
            >= params.min_cvss
        )
...
async def grimoire_search_playbook(params: PlaybookSearchInput, ctx: Context) -> str:
    ...
    if (
        not params.query
        and not params.mitre_technique_id
        and not params.severity
        and not params.phase
        and not params.platform
        and not params.log_source
    ):
        return _err(
    ...
    if params.phase:
        conditions.append(
            _jsonb_array_contains(
                Document.security_metadata, "playbook_phase", params.phase
            )
        )
    if params.platform:
        conditions.append(
            _jsonb_array_contains(
                Document.security_metadata, "platforms", params.platform.lower()
            )
        )
    if params.log_source:
        conditions.append(
            _jsonb_array_contains(
                Document.security_metadata, "log_sources", params.log_source.lower()
            )
        )
    ...
    async with get_db_context() as db:
        if ct == ContentType.SUMMARY:
            result = await agent.generate_summary(
                db, params.document_ids, style=params.style or "concise"
            )
        elif ct == ContentType.FLASH_CARD:
            result = await agent.generate_flash_cards(
                db, params.document_ids, count=params.count
            )
            ...
            result = await agent.generate_extract(
                db, params.document_ids, query=params.query
            )
        else:
            ...
            "documents": [
                {
                    ...
                    "file_type": (
                        d.file_type.value
                        if hasattr(d.file_type, "value")
                        else str(d.file_type)
                    ),
                    "processing_status": (
                        d.processing_status.value
                        if hasattr(d.processing_status, "value")
                        else str(d.processing_status)
                    ),
    ...
    async with get_db_context() as db:
        ...
        if params.parent_slug:
            parent = (
                (
                    await db.execute(
                        select(Category).where(Category.slug == params.parent_slug)
                    )
                )
                .scalars()
                .first()
...
async def grimoire_delete_document(params: DeleteDocumentInput, ctx: Context) -> str:
    ...
    if vector_ids:
        try:
            ...
            logger.debug(
                f"Vector store service not available, skipping cleanup for {params.document_id}"
            )
        except Exception as e:
            logger.warning(
                f"Vectors not cleaned up for {params.document_id}: {e}. May require manual ChromaDB cleanup."
            )
