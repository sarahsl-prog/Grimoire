...
class DocumentParser:
    ...
    IMAGE_EXTENSIONS: set[str] = {".png", ".jpg", ".jpeg", ".tiff", ".tif", ".gif", ".bmp", ".webp"}
    ...
    def _process_docling_result(
        ...
        try:
            ...
            if hasattr(result, 'document') and result.document is not None:
                ...
                # Check if markdown is actually a string, not a Mock
                md_value = result.markdown
                if isinstance(md_value, str):
                    text = md_value
                elif hasattr(md_value, '__str__') and not hasattr(md_value, '_mock_name'):
                    # Real object with string representation
                    text = str(md_value)
            else:
                # Fallback: try to get text from other attributes
                if result is not None:
                    text_str = str(result)
                    # Check if it's a real string or a mock
    ...
    async def parse(
        self,
        file_path: str | Path,
        custom_config: ParserConfig | None = None
    ) -> ParsedDocument:
        ...
        try:
            ...
            # Update file hash in result metadata if not set
            if result.metadata.file_hash is None or result.metadata.file_hash == "":
                result.metadata.file_hash = file_hash

            if result.status == "success":
                logger.info(f"Successfully parsed {file_path_obj}")
            elif result.status == "partial":
                logger.warning(f"Partially parsed {file_path_obj}: {result.error_message}")
            else:
