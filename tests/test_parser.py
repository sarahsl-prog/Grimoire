...
def parser_config() -> ParserConfig:
    ...
    """
    return ParserConfig(ocr_enabled=True, parse_images=False, timeout=30)
...
class TestParsedDocument:
    ...
    def test_basic_creation(self) -> None:
        ...
        doc = ParsedDocument(text="Sample text", status="success")
    ...
    def test_full_document_creation(self) -> None:
        ...
        metadata = DocumentMetadata(
            title="Test Title", author="Test Author", pages=10, word_count=500
        )
        doc = ParsedDocument(
...
class TestDocumentMetadata:
    ...
    def test_additional_fields(self) -> None:
        ...
        meta = DocumentMetadata(
            title="Test", custom_field="custom value", additional={"key": "value"}
        )
...
class TestParserConfig:
    ...
    def test_custom_config(self) -> None:
        ...
        config = ParserConfig(
            ocr_enabled=False, parse_images=True, enable_tables=False, timeout=60
        )
...
class TestDocumentParserAsync:
    ...
    async def test_parse_nonexistent_file(self, parser: DocumentParser) -> None:
        ...
        assert (
            ...
            or "File not found" in result.error_message
        )
...
class TestDocumentParserProcessResult:
    ...
    def test_process_successful_result(
        self, parser: DocumentParser, mock_docling_result: MagicMock, sample_pdf: Path
    ) -> None:
        """Test processing a successful conversion result."""
        result = parser._process_docling_result(
            mock_docling_result, sample_pdf, "test_hash"
        )
    ...
    def test_process_result_no_document(
        self, parser: DocumentParser, sample_pdf: Path
    ) -> None:
        ...
        result = parser._process_docling_result(mock_result, sample_pdf, "test_hash")
...
class TestDocumentParserWithMocks:
    ...
    async def test_parse_with_mocked_docling(
        self, parser: DocumentParser, sample_pdf: Path, mock_docling_result: MagicMock
    ) -> None:
        """Test parsing with mocked Docling converter."""
        with (
            patch("grimoire.core.parser.DOCLEY_AVAILABLE", True),
            patch.object(
                parser,
                "_get_converter",
                return_value=MagicMock(convert=lambda path: mock_docling_result),
            ),
        ):
    ...
    async def test_parse_timeout(self, parser: DocumentParser) -> None:
        ...
        try:
            with patch.object(
                parser, "_parse_sync", side_effect=lambda x, y: asyncio.sleep(10)
            ):
...
class TestParseDocumentFunction:
    ...
    async def test_parse_document_custom_config(self) -> None:
        ...
        result = await parse_document(
            "/nonexistent.pdf", ocr_enabled=False, parse_images=True
        )
...
class TestDocumentParserStateManagement:
    ...
    def test_config_immutability(self) -> None:
        ...
        assert (
            parser.config.ocr_enabled is False
        )  # Parser sees the change (same reference)
